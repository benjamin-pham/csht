...
- **Dữ liệu phân tán và độ trễ cao:** Toàn bộ dữ liệu giao dịch POS tại 34 chi nhánh (theo phạm vi/giả định của bài thực hành) của Ways Station hiện được xử lý thủ công và lưu trữ phân tán, dẫn đến độ trễ tổng hợp dữ liệu từ 48 đến 72 giờ.
...
Các KPI trong file thực hành được đặt ở mức **module/pilot** nhằm kiểm soát chặt chất lượng mô hình và dịch vụ dự báo. Khi đối chiếu với đồ án tổng thể, một số KPI có thể khác ngưỡng do được đo ở **mức nền tảng/kiến trúc tổng thể**. Vì vậy, không xem các ngưỡng này là mâu thuẫn mà là KPI theo từng cấp đo lường.
...
|:---|:---|:---|
| FR-01 | Trích xuất dữ liệu POS tự động | Kết nối database POS tại 34 chi nhánh (theo phạm vi/giả định của bài thực hành) qua kênh mã hóa TLS (Transport Layer Security), trích xuất giao dịch theo lịch batch hàng đêm (01:00 AM). |
...
| **Hiệu năng** | Thời gian xử lý Batch Job (34 chi nhánh (theo phạm vi/giả định của bài thực hành)) | ≤ 30 phút | KPI B.1 — Batch Processing |
...
3. **Dự báo (02:00 AM):** AI Engine chạy Batch Inference cho 34 chi nhánh (theo phạm vi/giả định của bài thực hành) dựa trên dữ liệu mới nhất, lưu trữ kết quả.
...
1. **Trích xuất dữ liệu (Extract):**
   - Hàng ngày vào lúc 01:00 AM, Airflow thực thi tác vụ trích xuất dữ liệu theo lô (batch) từ hệ thống Cơ sở dữ liệu POS của 34 chi nhánh (theo phạm vi/giả định của bài thực hành).
...
| **5. Triển khai & Vận hành (Rollout/Ops)** | T14–T15 | Triển khai Kubernetes; Cấu hình Grafana/Prometheus; Pilot 5 chi nhánh $\rightarrow$ Rollout 34 chi nhánh (theo phạm vi/giả định của bài thực hành). | Hệ thống Live; Dashboard giám sát; Tài liệu vận hành (Runbook). | Uptime $\ge$ 99.9%; Freshness $\le$ 24h. | Kỹ sư DevOps/SRE |

*Tổng thời lượng: 16 tuần (~4 tháng), trong đó tuần 16 dành cho ổn định sau rollout, bàn giao và đóng dự án.*
...
| R04 | **Nghẽn tải hệ thống lúc 08:00 AM:** 34 chi nhánh (theo phạm vi/giả định của bài thực hành) cùng truy cập API lấy dự báo gây rớt dịch vụ. | Thấp | Cao | **Vừa** | Thiết lập cơ chế Auto-scaling trên Kubernetes; Áp dụng Rate Limiting tại API Gateway. | SRE / DevOps |
...
*   **Kiểm thử Mô hình (Model Testing):** Chạy Backtesting trên dữ liệu lịch sử. Tiêu chí Pass: MAPE $< 10\%$ (KPI nghiêm ngặt cho module AI/pilot), MAE $\le 3$ và Precision $\ge 85\%$.
...
*   **Độ ổn định của Model:** Model có duy trì được MAPE $< 10\%$ (KPI nghiêm ngặt cho module AI/pilot) qua các tháng hay không, từ đó đánh giá tần suất cần thiết phải tái huấn luyện mô hình.
