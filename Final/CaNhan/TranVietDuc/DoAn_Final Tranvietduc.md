...
- **Điểm nghẽn 1 (Tích hợp & Vận hành Lớp 4 - App & Integration Platform):** 34 chi nhánh (theo phạm vi dữ liệu của đồ án) sở hữu 8 hệ thống phần mềm rời rạc (POS - Point of Sale, Bida, Gym, Kho...) hoạt động độc lập và phân mảnh. Tỉ lệ luồng tích hợp tự động là **0%**. 100% giao tiếp đi qua 3 kênh thủ công (Tổng đài, Zalo, Sổ giấy).
...
- **Rủi ro tuân thủ (Vấn đề 3 - Lỗ hổng bảo mật PII):** Việc lưu trữ dữ liệu cá nhân (PII) như ảnh CCCD, Face ID phân tán trên điện thoại cá nhân vi phạm nghiêm trọng **Nghị định 13/2023/NĐ-CP** về bảo vệ dữ liệu cá nhân. Doanh nghiệp đối mặt với rủi ro tuân thủ và xử lý vi phạm theo các quy định về bảo vệ dữ liệu cá nhân, đồng thời có nguy cơ ảnh hưởng uy tín thương hiệu.
...
|:---|:---|:---|:---|:---|
| TR-P01 | **[Lớp 4]** API Gateway xử lý đồng thời (concurrent connections) tại Cloud | ≥ 2.000 kết nối đồng thời (tính cho 34 chi nhánh × ~60 POS/thiết bị (theo giả định sizing của đồ án) + đệm 20%) | KPI 1.1 — Uptime | Load test đạt 2.000 concurrent connections, error rate < 0.1% |
...
| TR-P03 | **[Lớp 5]** Throughput Data Pipeline (CDC Streaming) | ≥ 5.000 events/giây (tổng 34 chi nhánh (theo phạm vi đồ án), giờ cao điểm) | KPI 2.1 — Data Freshness ≤ 5s | Stress test pipeline đạt throughput mục tiêu, không mất sự kiện |
...
> **Ghi chú về chi phí:** Các con số CAPEX/OPEX trong báo cáo là **dự toán phục vụ đồ án**, không phải báo giá chính thức từ nhà cung cấp. Chi phí Cloud là chi phí theo mức sử dụng và cần được tính lại bằng công cụ định giá của nhà cung cấp tại thời điểm triển khai. Khi triển khai thực tế, nên lấy tối thiểu 2–3 báo giá để xác nhận giá thiết bị, license và dịch vụ.
...
| Giảm rủi ro tuân thủ về bảo vệ dữ liệu cá nhân | — | Không lượng hoá, nhưng rủi ro rất lớn nếu bị kiểm tra |
| **Tổng lợi ích quy đổi / năm** | **~475.000.000** | |

> **Payback Period đơn giản (gross) ước tính:** $\frac{\text{CAPEX}}{\text{Lợi ích quy đổi/năm}} = \frac{1.308.800.000}{475.000.000} \approx$ **2,75 năm**. Đây là chỉ số tham khảo theo lợi ích quy đổi, **chưa trừ OPEX hằng năm**, nên không được hiểu là thời gian hoàn vốn tài chính ròng.

> **Phạm vi chi phí:** Mức đầu tư của đồ án cao hơn phần thực hành do đồ án bao gồm phạm vi chuỗi 34 chi nhánh, Edge Cluster, Kafka/CDC, Kubernetes HA, Data Lakehouse, bảo mật, giám sát, nhân sự vận hành và DR. Vì vậy không dùng số CAPEX/OPEX của file thực hành để thay thế trực tiếp cho ngân sách của đồ án.
...
> **Biện luận:** Hybrid Cloud có TCO trung bình nhưng là phương án duy nhất đáp ứng **100% tiêu chí kỹ thuật bắt buộc** (6/6 tiêu chí). On-Premise rẻ OPEX nhưng CAPEX quá cao và không co giãn. Cloud-Native rẻ CAPEX nhưng OPEX cao nhất và **không đáp ứng đầy đủ** yêu cầu offline và các yêu cầu kiểm soát dữ liệu của dự án.
